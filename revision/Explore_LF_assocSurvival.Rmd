...
#nonoe
DownGenes<-tolower(downreg$symbol)
...
b <- as.numeric(mm$match.matrix[,1])
background<-tolower(res$symbol[b])
test.result<-testForGSE( toTest, DownGenes, background)
table.out<-as.data.frame(test.result)
x.down <-datatable(table.out, 
                 caption = "Hallmark genes sets enriched in at a FDR < 10% caption downregulated genes")
```
