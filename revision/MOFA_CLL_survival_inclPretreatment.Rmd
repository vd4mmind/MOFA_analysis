...
plotdir <- "~/Documents/MOFA/CLL_MOFA_data/Analysis/code4manuscript/Figures_CLL_survival_addClinical"
...
biomarkers <- c("IGHV", "TP53", "age",  "del17p13", "trisomy12", "sex","MethylationCluster",
                "del11q22.3", "del13q14", "SF3B1", "NOTCH1", "IC50beforeTreatment",
                 "doubling.times")
...
df_hc <- lapply(c("TTT", "OS"), function(survType) {
  ...
  Zcommon <- Z[commonPats,]
  survivalData <- survivalData[commonPats,]
  print(sum(survivalData[,2]))

  dataCommmon <- lapply(data, function(dd) dd[,commonPats])
  print(sapply(dataCommmon, function(dd) sum(apply(dd,2, function(pat) any(is.na(pat))))))
  ...
  FeatureList$clinicalCovariates <- covariatesCommon
  FeatureList$doublingTime <- covariatesCommon[,"doubling.times", drop=FALSE]
  FeatureList$pretreated <- covariatesCommon[,"IC50beforeTreatment", drop=FALSE]
  FeatureList$both <- covariatesCommon[,c("doubling.times","IC50beforeTreatment"), drop=FALSE]
  FeatureList$BothWithFactors <- cbind(covariatesCommon[,c("doubling.times","IC50beforeTreatment"), drop=FALSE],Zcommon[,-1])
...
summaryHc_main <- filter(summaryHc, survType == "Time to next treatment",
                         predictors %in% c(names(data), "all", "MOFA factors",
                                           "factor 1", "factor 7", "factor 8", "factors 1,7,8",
                                           "clinicalCovariates", "doublingTime", "pretreated", "both","BothWithFactors"))
...
# long barplot
pdf(file.path(plotdir, "HarrelsC_TTT_long.pdf"), width=5, height=7)
  ggplot(summaryHc_main, aes(x=predictors, y=x.mean, fill=predictors, group=predictors))+
    geom_bar(stat = "identity") + 
   coord_cartesian(ylim = c(0.5,0.85)) +
  # ggtitle(paste("Prediction of time to treatment"))+
...
summaryHc_main <- filter(summaryHc, survType == "Time to next treatment",
                         predictors %in% c(names(data), "all", "MOFA factors",
                                           "factor 1", "factor 7", "factor 8", "factors 1,7,8",
                                           "clinicalCovariates", "doublingTime", "pretreated", "both"))
limits <- aes(ymax = summaryHc_main$x.mean + summaryHc_main$se,
              ymin = summaryHc_main$x.mean - summaryHc_main$se)
summaryHc_main$predictors <- factor(summaryHc_main$predictors,
                                    levels =c("Methylation", "Mutations","mRNA",  "Drugs",
...
summaryHc_supp$predictors <- factor(sapply(summaryHc_supp$predictors, function(nm) sub("_full","",nm)),
                                    levels =c("Methylation", "Mutations","mRNA",  "Drugs",
                                               "all", "MOFA factors"))#, "factors 1,7,8"))
...
## OS
```{r}
summaryHc_main <- filter(summaryHc, survType == "Overall Survival",
                         predictors %in% c(names(data), "all", "MOFA factors",
                                           "factor 1", "factor 7", "factor 8", "factors 1,7,8",
                                           "clinicalCovariates", "doublingTime", "pretreated", "both"))
limits <- aes(ymax = summaryHc_main$x.mean + summaryHc_main$se,
...
# barplot
pdf(file.path(plotdir, "HarrelsC_OS.pdf"))
  ggplot(summaryHc_main, aes(x=predictors, y=x.mean, fill=predictors, group=predictors))+
    geom_bar(stat = "identity") + 
   coord_cartesian(ylim = c(0.5,0.85)) +
  # ggtitle(paste("Prediction of overall survival"))+
  scale_fill_manual(values=cols4survival, labels=names(cols4survival)) + 
  geom_errorbar(limits, position = "dodge", width = 0.25)+
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size=18),
        axis.text.y = element_text(size=18),
...
# long barplot
pdf(file.path(plotdir, "HarrelsC_OS_long.pdf"), width=5, height=7)
  ggplot(summaryHc_main, aes(x=predictors, y=x.mean, fill=predictors, group=predictors))+
    geom_bar(stat = "identity") + 
   coord_cartesian(ylim = c(0.5,0.85)) +
  # ggtitle(paste("Prediction of overall survival"))+
