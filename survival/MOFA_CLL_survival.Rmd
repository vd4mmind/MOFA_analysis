...
ls_surv <- lapply(c("TTT", "OS"), function(survType) { 
  ...
  #range standardize continous predictor (latent factors and age) to have a comparable scale to binary predictors
  # rangestd <- function(x){(x-min(x))/(max(x)-min(x))}
  # Zcommon <- apply(Zcommon,2,rangestd)
  
  covariatesCommon <- covariates[commonPats,]
  # covariatesCommon[,"age"] <- rangestd(covariatesCommon[,"age"])
  ...
  # fit a cox model for latent factors
  ...
  colnames(pval_LFs) <- paste("LF",colnames(Z)[which(apply(Z,2,var, na.rm=T)>0)])

  # fit a cox model for other relevant biomarkers
  ...
  # df_survival$predictor <- factor(df_survival$predictor, levels = df_survival$predictor[order(df_survival$coef)])

  df_survival$survType <- ifelse(survType=="TTT", "Time to next treatment", "Overall Survival")
  df_survival$label <- paste(formatC(df_survival$p, digits = 2, format= "e"), sep="")
  df_survival
})
...
Seperate for LF and others
```{r}
ls_surv[[1]] %<>% mutate( predictorType = as.factor(predictorType))
ls_surv[[1]] %<>% mutate( positive = coef>1)

gg1 <- ggplot(filter(ls_surv[[1]], predictorType=="MOFA factor"), 
              aes(x=predictor, y=ifelse(positive,coef, 1/coef),ymin=ifelse(positive,lower, 1/lower),ymax=ifelse(positive,higher, 1/higher), col=predictorType))+
  geom_pointrange()+ coord_flip() +
...
gg2 <- ggplot(filter(ls_surv[[1]], predictorType!="MOFA factor"), aes(x=predictor,y=coef,ymin=lower,ymax=higher, col=predictorType))+
  ...
  guides(colour=guide_legend("type of predictor")) +
  xlab("")+  scale_color_discrete(drop=FALSE)
pdf(file.path(plotdir, "ForestPlotSurvival_TTT_LF.pdf"), width = 7, height = 6)
gg1
dev.off()
pdf(file.path(plotdir, "ForestPlotSurvival_TTT_Cov.pdf"), width = 7.5, height = 6)
gg2
dev.off()
```
...
# df_hc <- lapply(c("TTT", "OS"), function(survType) {
  ...
  # subset to common patients
   commonPats <- intersect(rownames(Z), rownames(survivalData))
  # also doing better on patients having all assays
  ...
  # FeatureList_all$LF_imputed <-  apply(Zcommon[,-1],2, function(x) { x[is.na(x)] <- mean(x, na.rm=TRUE); return(x)})
...
## Supplement: All features as predictors
```{r}
...
for(survType in c("TTT", "OS")){
 ...
 glist <- lapply(as.character(1:(ncol(Z)-1)), function(lfno){
  ...
  pdf(file.path(plotdir, paste("KaplanMeier_",survType,"_signif.pdf", sep="")), width=9, height=5)
