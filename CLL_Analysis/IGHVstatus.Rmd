...
gg_bees <- ggplot(df_clustering, aes(x=1,y=Z, col=agreement, label=ifelse(outlier,patID2, NA))) + geom_point(size=2, position = pos) +
  ...
  theme(axis.text.x = element_blank(),
        ...
        text = element_text(size=18))+
  guides(col=F) + ylab("Factor 1") + 
  ggrepel::geom_label_repel(cex=5, position=pos,
                        col=ifelse(df_clustering$outlier,patcols[df_clustering$patID2], "black"),
                        box.padding=4, point.padding=0.3, label.size=0)
gg_bees
```
...
# Arrange plots
```{r IGHV_mismatch, fig.height=15, fig.width=10}
...
c3 <- plot_grid(gg_drug +guides(col=F), gg_WES , labels = letters[5:6], nrow=1, rel_widths = c(1,1), label_size = 18)
