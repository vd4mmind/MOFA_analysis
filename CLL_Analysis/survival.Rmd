...
# Introduction
This script analyses the links of the MOFA factors to clinical outcome as shown in Figure 5 of the manuscript.
...
# df_hc <- lapply(c("TTT", "OS"), function(survType) {
  ...
  covariatesCommon <- covariates[commonPats,]
  # selected <- covariatesCommon[,c("TP53", "del17p13", "IGHV", "MethylationCluster", "SF3B1")]
  # commonPats <- intersect(commonPats, rownames(selected)[apply(selected,1,function(r) !any(is.na(r)))])
  # covariatesCommon <- covariates[commonPats,]
  # selected <- covariatesCommon[,c("TP53", "del17p13", "IGHV", "MethylationCluster", "SF3B1")]
  Zcommon <- Z[commonPats,]
  survivalData <- survivalData[commonPats,]
  ...
  # FeatureList$selected <- cbind(selected$TP53, ifelse(selected$IGHV=="U",0,1), ifelse(selected$MethylationCluster=="LP",0, ifelse(selected$MethylationCluster=="IP",1,2)), selected$SF3B1, selected$del17p13)
