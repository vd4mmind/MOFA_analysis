...
p <- ggplot(tmp, aes(x=model, y=k)) +
  ...
  # scale_y_continuous(breaks = c(0,6,7,8,9,10,11)) +
...
pdf("/Users/ricard/CLL/robustness_across_trials/out/K.pdf", width=7, height=5)
print(p)
dev.off()
...
weights <- lapply(1:length(models), function(i) {
  model <- models[[i]]
  Wtmp <- getWeights(model, views="mRNA", factors="all")
  colnames(Wtmp) <- paste(paste0("LF", 1:(ncol(Wtmp))), paste("model", i, sep=""), sep="_")
  Wtmp
  })

corWeights <- abs(cor(Reduce(cbind,weights)))
pdf("/Users/ricard/CLL/robustness_across_trials/out/robustness_weights.pdf", width=7, height=5)
