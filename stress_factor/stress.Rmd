...
foobar <- foobar[!drug %in% c("everolimus","BAY 11-70","SGI-1776","sunitinib","chaetoglobosin A","chaetocin","vorinostat","TAE684","SGX-523","saracatinib","KU−60019","AZD7762","gefitinib","encorafenib","rabusertib")]
...
foobar <- foobar[factor==stress_factor]
...
asd <- foobar[,.(weight=mean(weight)),by=c("drug","category","target","pathway")] %>%
  .[,weight:=abs(weight)] %>% setkey(weight) %>% tail(n=20) %>% .[,drug:=factor(drug,levels=drug)]
...
pdf("/Users/ricard/CLL/biology/out/stress/drug_weights.pdf", width = 8.5, height = 6, useDingbats = F)
print(p)
dev.off()
```
...
p <- ggplot(tmp, aes(x=conc, y=viab)) +
  # stat_summary(aes(group=interaction(drug,cluster), color=cluster), fun.data=f, geom="errorbar")  +
  stat_summary(aes(group=interaction(drug,cluster), color=cluster), fun.y="mean", geom="line", size=0.75)  +
  # stat_summary(aes(group=interaction(drug,cluster), color=cluster), fun.data="mean", geom="line")  +
