...
Alternatives: PC on methylation data vs LF colored by MC.
```{r, evl=F, echo=F}
# viewnm <- "Methylation"
...
```{r}
filter(df_clustering, grepl("clustered",agreement)) %>% select(IGHV, ZIGHV, patID, Z, MC)
```
...
# Imputations based on single omics
...
plotPCA <- function(viewnm){
  ...
  PCA_out %<>% mutate(disagreement = grepl("clustered", agreement))
    
  gg1 <- ggplot(PCA_out, aes(x=PC1, y=PC2, col=ZIGHV, shape =disagreement)) +geom_point() +
  scale_color_manual(values=anno_colors$IGHV_predicted)  + coord_fixed()+scale_shape_manual(values=c("TRUE"=17, "FALSE"=19))+
    ggtitle(viewnm)
  gg2 <- ggplot(PCA_out, aes(x=PC1, y=PC2, col=IGHV, shape =disagreement)) +geom_point() +
  scale_color_manual(values=anno_colors$IGHV_label) + coord_fixed() +scale_shape_manual(values=c("TRUE"=17, "FALSE"=19))+
    ggtitle(viewnm)
...
anno_df <- data.frame(Z=Z[,idx_ighv])#, ZIGHV=ZIGHV)
