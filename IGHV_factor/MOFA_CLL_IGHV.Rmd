...
plotdir <- "~/Documents/MOFA/CLL_MOFA_data/Analysis/code4manuscript/Figures_CLL_IGHV"
...
IGHV <- as.factor(covariates[,"IGHV"])
IGHV <- ifelse(IGHV==0, 16, ifelse(IGHV==1, 1,2 ))
...
#PC on methylation data vs LF
```{r}
# viewnm <- "Methylation"
# haveOmic <- apply(model@TrainData[[viewnm]],2,function(p) !any(is.na(p)))
# pc.out <- prcomp(t(model@TrainData[[viewnm]][,haveOmic]))
# PC1 <- pc.out$x[,1]
# df <- data.frame(PC1=PC1, Z=Z[haveOmic,"1"], MC=MC[haveOmic])
# pdf(file.path(plotdir,"LFvsPC_meth.pdf"),width=5, height=8)
# ggplot(df, aes(x=Z, y=PC1, col=MC)) +geom_point() +ylab("PC1 of methylation data")
# dev.off()
```



# Classification and Agreeemnt with label
...
# ZIGHV <- ifelse(Z[,idx_ighv]<0.6, "M", "U")
...
scatterPlot(model, c("1", "2"), color_by = ZIGHV, shape_by = IGHV)

# save(df_clustering, file="df_clustering.RData")
...
Classification based on other omics
```{r}
imputePerOmic <- function(viewnm){
haveOmic <- apply(model@TrainData[[viewnm]],2,function(p) !any(is.na(p)))

# cluster using kmeans to determine 2 IGHV factor/cluster
set.seed(23130)
omic_IGHV <- kmeans(prcomp(t(model@TrainData[[viewnm]][,haveOmic]))$x[,1], 2, iter.max = 1000, nstart = 1000)
omic_IGHV <- omic_IGHV$cluster
omic_IGHV[omic_IGHV==1] <- "U"
omic_IGHV[omic_IGHV==2] <- "M"

df_clust <- data.frame(IGHV = as.factor(IGHV), 
                            MC = as.factor(MC), patID = rownames(Z),
                            Z = Z[,idx_ighv])
omic_IGHV <- sapply(rownames(df_clust), function(p) if(p %in% names(omic_IGHV)) omic_IGHV[p] else NA)
df_clust$omic_IGHV <- omic_IGHV
rm(omic_IGHV)

if(sum(df_clust$omic_IGHV[df_clust$IGHV!="missing"]==df_clust$IGHV[df_clust$IGHV!="missing"], na.rm = TRUE)<sum(df_clust$omic_IGHV[df_clust$IGHV!="missing"]!=df_clust$IGHV[df_clust$IGHV!="missing"], na.rm = TRUE)) df_clust$omic_IGHV <- ifelse(df_clust$omic_IGHV=="M", "U","M")

#check agreement between clusters
omic_clagree <- paste(df_clust$omic_IGHV , df_clust$IGHV, sep="_")
omic_clagree <- ifelse(omic_clagree %in% c("M_M", "U_U"), "agreement with label",
                  ifelse(grepl("NA_missing",omic_clagree), "both labels missing",
                         ifelse(grepl("NA_",omic_clagree), "omic label missing",
                                ifelse(grepl("_missing",omic_clagree), "clinical label missing",
                         ifelse(omic_clagree=="M_U", "U-CLL clustered as M-CLL", "M-CLL clustered as U-CLL" )))))
df_clust$omic_agreement <- omic_clagree


colors_agreemet_omic <- c("agreement with label" = "darkgreen", 
                               "clinical label missing" ="gray", 
                               "U-CLL clustered as M-CLL" ="sienna",
                               "M-CLL clustered as U-CLL" = "orange",
                                "omic label missing" = "black",
                           "both labels missing" = "burlywood")

#pie plot
df_pie_omic <- df_clust %>% group_by(omic_agreement) %>% dplyr::summarize(value=length(patID))
df_pie_omic$agreement <- factor(df_pie_omic$omic_agreement, levels = rev(df_pie_omic$omic_agreement))

gg <- ggplot(df_pie_omic, aes(x="", y=value, fill=agreement)) +
  geom_bar(stat="identity",width = 1)+
  coord_polar("y", start=0)+
  theme_minimal() + xlab("") + ylab("")+
  scale_fill_manual(values=colors_agreemet_omic) +
  ...
  scale_color_manual(values=colors_agreemet_omic) +
  ggtitle(viewnm)
...
```{r}
gg_mRNA <- imputePerOmic("mRNA")
gg_meth <-imputePerOmic("Methylation")
gg_viab <-imputePerOmic("Drugs")
```
...
```{r}
pdf(file.path(plotdir,"Class_comparison_pies.pdf"))
grid.arrange(gg_MOFA +guides(fill=F) + ggtitle("MOFA"), gg_mRNA+guides(fill=F), gg_meth+guides(fill=F), gg_viab+guides(fill=F), get_legend(gg_mRNA))
dev.off()
```

# IGHV classification and clustering on various biological layers
```{r}
#set nice name
df_clustering$IGHV_predicted <- df_clustering$ZIGHV
df_clustering$IGHV_label <- df_clustering$IGHV
df_clustering$haveRNAseq <- ifelse(apply(data$mRNA, 2, function(p) !all(is.na(p)))[rownames(df_clustering)],
                                   "yes", "no")

survivalDataTTT <- as.matrix(patmeta[,c("T5","treatedAfter")])
survivalDataTTT <- survivalDataTTT[!is.na(survivalDataTTT[,1]),]
df_clustering$haveTTT <- ifelse(rownames(df_clustering) %in% rownames(survivalDataTTT),
                                   "yes", "no")

#callbakc function to order as much as possible in agreement with continious Z
...
# Omic-wise heatmaps
## Drugs
pdf(file.path(plotdir, "HeatmapDrugs_IGHV.pdf"), width = 8, height = 7, onefile = F)
drugData <- data$Drugs
...
pheatmap(cor(drugData),
         ...
         show_rownames = F, show_colnames = F, main = "Clustering based on drug response data",
...
## Methylation
pdf(file.path(plotdir, "HeatmapMeth_IGHV.pdf"), width = 8, height = 7, onefile = F)
methData <- data$Methylation
methData <- methData[,apply(methData,2, function(d) !all(is.na(d)))]
pheatmap(cor(methData),
         annotation_row = select(df_clustering, c(IGHV_label, IGHV_predicted, agreement)),
         show_rownames = F, show_colnames = F, main = "Clustering based on methylation data",
         annotation_colors  =  anno_colors, annotation_legend = F, treeheight_col=0#,
          #clustering_callback = orderbyZ
         )
dev.off()
...
## mRNA
pdf(file.path(plotdir, "HeatmapRNAseq_IGHV.pdf"), width = 8, height = 7, onefile = F)
mRNAData <- data$mRNA
mRNAData <- mRNAData[,apply(mRNAData,2, function(d) !all(is.na(d)))]
pheatmap(cor(mRNAData),
         annotation_row = select(df_clustering, c(IGHV_label, IGHV_predicted, agreement)),
         show_rownames = F, show_colnames = F, main = "Clustering based on mRNA data",
         annotation_colors  =  anno_colors, annotation_legend = F, treeheight_col=0#,
         #clustering_callback = orderbyZ
         )
dev.off()
...
plotPCA <- function(viewnm){
  ...
  PCA_out <- cbind(PCA_out,  Zdf[rownames(PCA_out),])
  
  gg1 <- ggplot(PCA_out, aes(x=PC1, y=PC2, col=ZIGHV, shape =disagreement)) +geom_point() +
  scale_color_manual(values=anno_colors$IGHV_predicted)  + coord_fixed()+scale_shape_manual(values=c("TRUE"=3, "FALSE"=19))+
    ggtitle(viewnm)
  gg2 <- ggplot(PCA_out, aes(x=PC1, y=PC2, col=IGHV, shape =disagreement)) +geom_point() +
  scale_color_manual(values=anno_colors$IGHV_label) + coord_fixed() +scale_shape_manual(values=c("TRUE"=3, "FALSE"=19))+
    ggtitle(viewnm)
...
grid.arrange(gg_meth[[1]],  gg_meth[[2]], gg_mRNA[[1]],
             gg_mRNA[[2]], gg_drugs[[1]], gg_drugs[[2]])
dev.off()
...
ntop_mRNA <- 20
knownGenes <- c("AKAP12", "ADAM29", "BCL7A", "CLECSF2", "FCGBP", "FLJ10884", "FUT8", "LPL", "TCF7", "WNT3", "APOD", "SPG20", "MYL9", "NRIP1", "SPAP1", "SPRY2", "TGFBR3", "ZAP70", "COBLL1", "ZNF667", "SEPT10", "CRY1", "PLD1", "BCL7A", "WNT9A")
pdf(file.path(plotdir,"mRNAweigths_IGHV.pdf"),width=5, height=8)
showTopWeightsAndColor(model, "mRNA", idx_ighv, nfeatures = ntop_mRNA, Features2color = knownGenes, scalePerView = T, col2highlight = "darkorange")
dev.off()                                                                                                                               
```
...
# drugW <- getWeights(model, "Drugs", "all")
# drugW <- drugW/max(abs(drugW))
# drugW <- drugW[,idx_ighv]
# dfDrugs <- as.data.frame(drugW)
...
pdf(file.path(plotdir,"drugweigths_IGHV.pdf"),width=7, height=5)
...
anno_df <- data.frame(Z=Z[,"1"])#, ZIGHV=ZIGHV)
colnames(anno_df) <- c("Factor 1")#, "Clusters")
rownames(anno_df) <- rownames(Z)
annoHM_colors <- list(c("blue", "red"))#, c("M"="red", "U"="blue"))
names(annoHM_colors) <- c("Factor 1")#, "Clusters")

pdf(file.path(plotdir,"heatmap_mRNA_IGHV.pdf"),width=10, height=7, onefile = F)
...
pdf(file.path(plotdir,"heatmap_mRNA_IGHV_rotated.pdf"),width=7, height=8, onefile = F)
pheatmap(data$mRNA[rev(topGenes), patients2include[order(Z[patients2include, idx_ighv])] ],
         ...
         gaps_col = which(ZIGHV[order(Z[patients2include,idx_ighv], decreasing = F)]=="M")[1]-1,
         show_rownames = F, legend = F, annotation_colors = annoHM_colors)
dev.off()

pdf(file.path(plotdir,"heatmap_mRNA_IGHV_rotated2.pdf"),width=7, height=8, onefile = F)
pheatmap(data$mRNA[topGenes, patients2include[order(Z[patients2include, idx_ighv])] ], show_colnames = F, cluster_rows = F, cluster_cols = F, fontsize = 18, annotation_col = anno_df, annotation_legend = F, gaps_col = which(ZIGHV[order(Z[patients2include,idx_ighv], decreasing = F)]=="M")[1]-1,
         show_rownames = T, legend = T, annotation_colors = annoHM_colors)
dev.off()

pdf(file.path(plotdir,"heatmap_mRNA_IGHV_legend.pdf"),width=13, height=7, onefile = F)
...
methData <- data$Methylation
methData <- methData[,apply(methData,2, function(d) !all(is.na(d)))]
pdf(file.path(plotdir, "Heatmap_Meth_small.pdf"), width = 10, height = 7, onefile = F)
pheatmap(t(methData)[,order(apply(t(methData),2,var), decreasing = T)[1:100]],
         annotation_row = anno_df,
         show_rownames = F, show_colnames = F, main = "Clustering based on top variable methylation sites", annotation_legend =  F)
dev.off()
```
...
```{r}
source("plottingUtils.R")
# for(drugnm in c("dasatinib", "ibrutinib", "AT13387", "AZD7762")){
# pdf(file.path(plotdir, paste0("DrugResponseCurve_",drugnm,".pdf")), width = 7, height = 7, onefile = F)
# p <- plotDrugResponseCurve(model, drugnm, groups = ZIGHV, groupnm = "predicted IGHV") 
# p <- p + scale_color_manual(values = anno_colors$IGHV_predicted) 
# print(p)
# dev.off()
# }
for(drugnm in c("dasatinib", "AT13387", "ibrutinib")){
pdf(file.path(plotdir, paste0("DrugResponseCurve_",drugnm,".pdf")), width = 4, height = 6, onefile = F)
p <- plotDrugResponseCurve(model, drugnm, groups = ZIGHV, groupnm = "predicted IGHV")
p <- p + scale_color_manual(values = anno_colors$IGHV_predicted, labels=c("M-CLL", "U-CLL")) + ylim(c(0.2,1.05))
# print(p)
...
```{r}
  ...
  groups = ZIGHV
  groupnm = "predicted IGHV"
  drugResDF <- lapply(c("dasatinib", "ibrutinib"), function(drugnm) {
  ...
  p <- p + scale_color_manual(values = anno_colors$IGHV_predicted, labels=c("M-CLL", "U-CLL")) +
    ylim(c(0.2,1.05))
