...
inFiles=( "$inFolder/viab.txt" "$inFolder/mut.txt" "$inFolder/meth.txt" )
...
for Nmasked in "${Nmasked_list[@]}"; do
	for trial in $(seq 1 $ntrials); do
		outFile="/hps/nobackup/stegle/users/ricard/CLL/out/imputation/FullCases/7Sep/N${Nmasked}_norna_$trial.hdf5"
		cmd="mofa
			--inFiles ${inFiles[@]}
			--delimiter ' '
			--header_cols
