...
inFiles=( "$inFolder/viab.txt" "$inFolder/meth.txt" "$inFolder/mRNA.txt" "$inFolder/mut.txt" )

likelihoods=( gaussian gaussian gaussian bernoulli )
views=( viab meth mRNA mut )
...
for na in "${na_list[@]}"; do
	for trial in $(seq 1 $ntrials); do
		outFile="/hps/nobackup/stegle/users/ricard/CLL/out/imputation/missingAtRandom/7Sep/${na}_drugmetrnamut_$trial.hdf5"
		cmd="mofa
			--inFiles ${inFiles[@]}
			--outFile $outFile
			--delimiter ' '
