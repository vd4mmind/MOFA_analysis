...
```{r}
out_mofa <- "/Users/bvelten/Documents/MOFA/GFA_imputation_models/CLL/imputation/atRandom/mofa/k10/"
out_gfa <- "/Users/bvelten/Documents/MOFA/GFA_imputation_models/CLL/imputation/atRandom/gfa/"
...
sapply(seq_along(ListMOFA), function(i) getDimensions(ListMOFA[[i]])$K)
...
df <- data.frame(true=missing, name = names(missing), label=labelmissing, run=run)
rownames(df) <-NULL
head(df)
```
...
# check number of factors
sapply(ListGFA, function(m) ncol(m$X))
...
which(!Reduce(c,featureNames(ListMOFA[[1]])) %in% rownames(ListGFA[[1]]$W))
which(!rownames(ListGFA[[1]]$W) %in% Reduce(c,featureNames(ListMOFA[[1]])))

# change a name in MOFA
rownames(ListGFA[[1]]$W)[61]
(Reduce(c,featureNames(ListMOFA[[1]])))[9309]
...
save(benchmark_results, file=file.path(outdir,"imputation_random_drug_k10.RData"))
```
